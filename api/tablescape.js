...
export default async function handler(req, res) {
  ...
  let event, style, colors, guests, budget, notes;
  try {
        ...
        for (const item of cat.items) {
          ...
          try {
            ...
            if (sr.ok) {
              ...
              if (results.length > 0) {
                item.real_link = results[0].product_link || results[0].link || '';
                item.thumbnail = results[0].thumbnail || '';
                item.real_price = results[0].extracted_price || item.price;
              }
            }
          } catch (e) {}
