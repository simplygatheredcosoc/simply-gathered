        ...
        for (const item of cat.items) {
          ...
          try {
            ...
            if (sr.ok) {
              ...
              const allowed = ['anthropologie','serena','pottery barn','potterybarn','williams sonoma','williams-sonoma','west elm','westelm','crate','cb2','restoration hardware','rh.com','mcgee','one kings lane','onekingslane','arhaus','terrain','rejuvenation','schoolhouse','lulu and georgia','luluandgeorgia','burke decor','burkedecor','ballard','perigold','joss','allmodern','birch lane','wayfair','nordstrom','bloomingdale','neiman','saks','bergdorf','macy','sur la table','surlatable','food52','juliska','replacements','heath ceramics','east fork','year and day','le creuset','diptyque','voluspa','mackenzie-childs','lenox','kate spade','vietri','wedgwood','waterford','herend','shopbop','net-a-porter','goop','the citizenry','aerin','ralph lauren','design within reach','article','apt2b','etsy','chairish','1stdibs','ruby lane','ebay','target','amazon','world market','h&m home','hm.com','zara home','ikea','lumens','circa lighting','home depot','homedepot','lowes','minted','framebridge','john derian','brooklinen','parachute','boll','matouk','sferra','rugs usa','rugsusa','ruggable','loloi','dash & albert','google'];
              const ok = results.filter(r => { const s = (r.source||'').toLowerCase(); const l = (r.link||r.product_link||'').toLowerCase(); return allowed.some(a => s.includes(a) || l.includes(a)); }); });
              if (ok.length > 0) {
                item.real_link = ok[0].product_link || results[0].link || '';
                item.thumbnail = ok[0].thumbnail || '';
                item.real_price = ok[0].extracted_price || item.price;
              }
            }
          } catch (e) {}
