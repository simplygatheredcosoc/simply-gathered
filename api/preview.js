...
export default async function handler(req, res) {
  ...
  try {
    ...
    var customPrompt = body.customPrompt || null;
    var generateImageOnly = body.generateImageOnly || false;
  } catch (e) {
  ...
  try {
    // Fast path: designer concept image only
    if (generateImageOnly && customPrompt && OPENAI_KEY) {
      try {
        const dalleRes = await fetch('https://api.openai.com/v1/images/generations', {
          ...
          body: JSON.stringify({
            model: 'dall-e-3',
            prompt: customPrompt + ' Photorealistic, shot on Canon 5D, natural light, editorial interior photography, Architectural Digest style, no text or watermarks.',
            n: 1,
            size: '1792x1024',
            quality: 'standard',
            style: 'natural'
        ...
        if (dalleRes.ok) {
          ...
          if (dalleData.data && dalleData.data[0]) {
            return res.status(200).json({ generated_image: dalleData.data[0].url });
          }
        }
      } catch (e) {}
      return res.status(200).json({ generated_image: null });
    }
